...
export default function LoginPage() {
  ...
  // Where to land after sign-in. Only allow same-origin internal paths.
  const next = useMemo(() => {
  ...
  const sendOtpCode = async (e: React.FormEvent) => {
    ...
    setStatus('sending');
    setError(null);
    // shouldCreateUser:true + no emailRedirectTo → Supabase sends a 6-digit code.
    const { error: err } = await supabase.auth.signInWithOtp({
      email,
  ...
  return (
    <div className="mx-auto flex h-full max-w-md flex-col items-center justify-center px-6">
      <h1 className="mb-2 text-2xl font-semibold">Sign in to Mesh</h1>
      <p className="mb-8 text-sm text-neutral-400">EU-hosted · RGPD by design</p>
      ...
      <div className="mb-3 flex w-full rounded-md border border-neutral-800 p-0.5 text-xs">
        <button
          onClick={() => {
            setMode('magic-link');
            setStatus('idle');
          ...
          className={`flex-1 rounded px-3 py-1 ${
            mode === 'magic-link' ? 'bg-neutral-800 text-neutral-100' : 'text-neutral-400'
          }`}
        >
          Magic link
        </button>
        <button
          ...
          className={`flex-1 rounded px-3 py-1 ${
            mode === 'otp-code' ? 'bg-neutral-800 text-neutral-100' : 'text-neutral-400'
          }`}
        >
          6-digit code
        </button>
        ...
        <>
          {status === 'sent' ? (
            <p className="text-sm text-neutral-400">
              Check your email — we just sent a magic link to <strong>{email}</strong>.
            </p>
          ) : (
          ...
          {status === 'sent' || status === 'verifying' ? (
            <form onSubmit={verifyOtp} className="flex w-full flex-col gap-3">
              <p className="text-xs text-neutral-400">
                Code sent to <strong>{email}</strong>. Enter the 6-digit code:
              </p>
              <input
                inputMode="numeric"
                pattern="\d{6}"
            ...
            <form onSubmit={sendOtpCode} className="flex w-full flex-col gap-3">
              <input
                type="email"
                required
                placeholder="you@example.com"
              ...
              <button
                ...
                className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-600 disabled:opacity-50"
              >
                {status === 'sending' ? 'Sending…' : 'Send 6-digit code'}
              </button>
            </form>
          )}
        </>
